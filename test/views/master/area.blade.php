...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-16">Area List</h4>
            <div class="page-title-right">
                @if(App\Menurole::getMenuRole(Auth::user()->role_id,22,'create'))
                    <a class="btn btn-sm btn-outline-secondary" data-bs-toggle="collapse" href="#createArea" aria-expanded="false" aria-controls="createArea" onclick="$('#editid').val(0);">Create Area</a>
                @endif
            </div>
        </div>
    </div>
                        ...
                        <div class="row">
                                ...
                                <div>
                                    ...
                                    <select class="form-control" id="governorate_id" name="governorate_id">
                                        ...
                                        @foreach($governorates as $governorate)
                                            <option value="{{$governorate->id}}">{{$governorate->governorateenglish}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-4 mb-3">
                                <div>
                                    ...
                                    <input class="form-control" type="text" name="area_code" id="area_code" required="">
                                </div>
                            </div> 
...
<div class="row">                                
    <div class="table-responsive mt-4">
        ...
        <tr>
            <td>{{$list->id}}</td>
            <td>{{$list->governorate->governorateenglish ?? ''}}</td>
            <td>{{$list->name}}</td>
            <td>{{$list->name_ar}}</td>
            <td>{{$list->area_code}}</td>
            <td><a href="{{route('master.area.day.slots',[$list->id])}}" class="btn btn-sm btn-outline-success">Slot</a></td>
            <td>
                <div class="d-flex gap-3">
                    @if(App\Menurole::getMenuRole(Auth::user()->role_id,22,'edit'))
                        <a href="#" class="btn btn-sm btn-outline-secondary edit-list" data-id="{{$list->id}}" data-name="{{$list->name}}" data-governorate_id="{{$list->governorate_id}}" data-area_code="{{$list->area_code}}" data-name_ar="{{$list->name_ar}}">Edit</a>
                    @endif
                    @if(App\Menurole::getMenuRole(Auth::user()->role_id,22,'delete'))
                        <a href="{{route('master.area.delete',[$list->id])}}" class="btn btn-sm btn-outline-danger">Delete</a>
                    @endif
                </div>
            </td>
        </tr>
...
$(".edit-list").click(function() {
    ...
    $('#area_code').val($(this).data('area_code'));
