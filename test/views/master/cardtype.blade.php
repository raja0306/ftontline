...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-16">Cardtype List</h4>
            <div class="page-title-right">
                @if(App\Menurole::getMenuRole(Auth::user()->role_id,27,'create'))
                    <a class="btn btn-sm btn-outline-secondary" data-bs-toggle="collapse" href="#createArea" aria-expanded="false" aria-controls="createArea" onclick="$('#editid').val(0);">Create Cardtype</a>
                @endif
            </div>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12 pt-3">
                        <form  action="{{route('master.cardtype.store')}}" method="post">
                        {{csrf_field()}}  
                        <div class="row">
...
<div class="row">                                
    <div class="table-responsive mt-4">
        ...
        <tbody>
        @foreach ($cardtypes as $list)
        <tr>
            <td>{{$list->id}}</td>
            <td>{{$list->name}}</td>
            <td>
                <div class="d-flex gap-3">
                    @if(App\Menurole::getMenuRole(Auth::user()->role_id,27,'edit'))
                    ...
                    @endif
                    @if(App\Menurole::getMenuRole(Auth::user()->role_id,27,'delete'))
                        <a href="{{route('master.cardtype.delete',[$list->id])}}" class="btn btn-sm btn-outline-danger">Delete</a>
                    @endif
                </div>
            </td>
        </tr>
