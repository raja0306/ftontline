...
<div class="col-12">
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0 font-size-18">Incoming @if($status=='missed')- Missed @endif Details</h4>

        <div class="page-title-right">
            <ol class="breadcrumb m-0">
                <li class="text-muted">Call Logs > </li>
                <li class="text-muted">Incoming Calls</li>
            </ol>
        </div>
            ...
            <div class="col-sm-12 pt-3">
                <div class="row">
                    ...
                    <div class="col-lg-3 mt-3">
                        ...
                        <button type="button" class="btn btn-info waves-effect waves-light" id="resetbtn">Reset <i class="mdi mdi-arrow-right ms-1"></i></button>
                    </div>
                </div>
            </div>
        </div>
...
<div class="col-12">
    <div class="card">
        ...
        Missed  
        @if($status=='missed') 
            {!!App\VicidialCloserLog::NetMissedspan($log->phone_number,$log->call_date)!!} 
        @endif
                        </td>
                        <td>{{$log->user}}</td>
                        <td>{{App\Average::toMinutes($log->length_in_sec)}}</td>
                        <td>{{App\Average::toMinutes($log->queue_seconds)}}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
$('#submitbtn').click(function() {
    ...
    var submiturl = "{{url('/calls/missed')}}?status=missed&reset=false"+"&fromdate="+fdate+"&todate="+todate+"&phone="+phone+"&campaign="+campaign;
...
$('#resetbtn').click(function() {
    ...
    if (confirm_result == true) {
        ...
        var phone = $("#phone").val();
        var status = $("#status").val();
        var submiturl = "{{url('/calls/missed')}}?status=missed&reset=true"+"&fromdate="+fdate+"&todate="+todate+"&phone="+phone+"&campaign="+campaign;
        window.location.href = submiturl;
    }
});
</script>
