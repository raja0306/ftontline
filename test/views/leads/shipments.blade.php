        ...
        <div class="card-body">
            <div class="table-responsive">
                ...
                <tbody>
                    ...
                    <tr onclick="window.location='{{url('book/shipment/view/')}}/{{$log->id}}', '_blank'" style="cursor: pointer;">
                        <td>{{ date('Y-m-d',strtotime($log->pickup_date)) }}</td>
                        <td>{{ $log->reference }}</td>
                        ...
                        <td>{{ $log->cardtype->name ?? $log->description }}</td>
                        ...
                        @else 
                            <a target="_blank" href="{{url('/book/customer')}}/{{$log->consignee_phone_upload}}/{{$log->agent ?? 'admin'}}/1000/OUTIN/OUTIN/{{$log->lead_id ?? 0}}" class="btn btn-sm btn-outline-danger edit-list">Book</a>
                        @endif</td> -->
                    </tr>
                    @endforeach
                </tbody>
...
<script>
$(document).ready(function(){$("#datatable").DataTable(),$("#datatable-buttons").DataTable({lengthChange:!1,buttons:["copy","csv"]}).buttons().container().appendTo("#datatable-buttons_wrapper .col-md-6:eq(0)"),$(".dataTables_length select").addClass("form-select form-select-sm")});
var extravals = '';
