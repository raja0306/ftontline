...
class CustomerbookController extends Controller
{
   ...
   {
      $shipment = Shipment::where(function($query) use ($civil) {
        $query->where('customer_civil_Id', $civil)
              ->orWhere('receiver_civil_id', $civil);
    })
    ->first();

      if($shipment){
         return redirect('/booknew/customer/'.$shipment->consignee_phone_upload.'/'.$user.'/'.$listid.'/'.$campaignid.'/'.$ingroup.'/'.$leadid);
      }
      else{
   ...
   public function book($mobile,$user,$listid=0,$campaignid=0,$ingroup=0,$leadid=0,$appid=0)
   {
      ...
      $shipments = Shipment::where('consignee_phone_upload',$mobile)->where('appointment_id','0')->get();
      $appoint = null;
      if($appid!=0){
   ...
   public function booknew($mobile,$user,$listid=0,$campaignid=0,$ingroup=0,$leadid=0,$appid=0)
   {
      // $shipment_id = 24916;
      // echo app('App\Http\Controllers\ShipmentController')->shipmentinfo($shipment_id); exit();
      if (strlen($mobile) > 8) {
         $mobile = substr($mobile, -8);
      }
      ...
      return view('appointment.index',compact('mobile','user','listid','campaignid','ingroup','leadid','listname','lead','slots','areas','branches','lead','shipments','customer','customer_addresses','appointments','upload_id','enq_categories','enquiries','appid','appoint'));
   }
   ...
   public function store(Request $req)
   {
      ...
      if (!empty($req->shipment_ids)) {
      foreach ($req->shipment_ids as $shipment_id) {
          ...
          if($ship){
               ...
               if($apmt){
                  ...
                  $message_wa="Appointment Booked Message Sent";
                  if($req->edit_id>0){ 
                     $message="Appointment Edited";
                     $message_wa="Appointment Edited Message Sent";
                  }
                  app('App\Http\Controllers\ShipmentuploadController')->CreateShipmentinfo($shipment_id,$message,'CALLING ASSIGNED',$message);
                  if($apmt->address_type!=2){

                     // SendWhatsappAppointment::dispatch('98090263',$ship->consignee_name,date('d M,Y',strtotime($apmt->appointment_date)),$apmt->slot->name,$apmt->area->name,$apmt->useraddress->block,$apmt->useraddress->street,$apmt->useraddress->house_no,$apmt->useraddress->floor_no,$apmt->useraddress->flat_no,$apmt->useraddress->landmark);
                     app('App\Http\Controllers\ShipmentuploadController')->CreateShipmentinfo($shipment_id,$message_wa,'APPOINTMENT BOOKED',$message_wa);
                  }
               }
          }
   ...
   public function store_enquiry(Request $req)
   {
      ...
      $shipment_ids = $req->shipment_ids;
      if(!empty($req->shipment_ids)){ $shipment_ids = implode(",", $req->shipment_ids); }
      $app_id = Enquiry::insertGetId(['enq_id'=>$req->enq_cat_id,'lead_id'=>$req->lead_id,'mobile'=>$req->mobile,'user'=>$req->user,'description'=>$req->description,'shipment_ids'=>$shipment_ids,'appointment_date'=>$req->follow_date,'created_at'=>now()]);
      $encat = EnqCategory::where('id_enquiry_category',$req->enq_cat_id)->first();
      // echo $encat->is_block_shipment.' , '; print_r($shipment_ids); exit();

      if($encat){
         $enqmsg = 'Status enquiry added';
         $message="Enquiry status added";
         if($encat->is_block_shipment==1 && !empty($shipment_ids)){
            $enqmsg = 'Shipment blocked';
            Shipment::whereIn('id',explode(",", $shipment_ids))->update(['block_call'=>$req->enq_cat_id]);
         }
         if($encat->is_notifed==1){
            User::where('id',$req->user_id)->update(['flag'=>$encat->category_name]);
         }
         foreach ($req->shipment_ids as $shipid) {
            app('App\Http\Controllers\ShipmentuploadController')->CreateShipmentinfo($shipid,$message,$enqmsg,$message);
         }
      }
   ...
   public function calllogs($mobile)
   {
      ...
      $ihtml = view('appointment.call_logs',compact('incomings','outgoings'))->render();
   ...
   public function apptlogs($mobile)
   {
      ...
      $ihtml = view('appointment.appt_logs',compact('appointments','url','last5shipments'))->render();
      echo json_encode(["ihtml"=>$ihtml]);
   }
   ...
   public function date_apps($date=null)
   {
      ...
      // }
      $slothtml = '';
      $alreadyslot = \App\Areadayslot::where('area_id',$area_id)->where('day',$day)->where('delete_status',0)->first();
      if($alreadyslot){
         $slots = Slot::whereIn('id',explode(",",$alreadyslot->slot_ids))->where('delete_status',0)->get();
         $slothtml = view('appointment.slots',compact('slots','date'))->render();
      }
      $appointments = Appointment::where('appointment_date',$date)->where('delete_status',0)->count();
      $alert = ''; if($date==date("Y-m-d")){ $alert = 'Its a same day appointment'; }
      echo json_encode(["slothtml"=>$slothtml,"count"=>$appointments,"is_date"=>$hide_date,"editid"=>$editid,"alert"=>$alert]);
   }

   public function mark_customer($idval,$flag=null)
   {
      User::where('id',$idval)->update(['flag'=>$flag]);
      echo json_encode(["flag"=>ucfirst($flag)]);
   }
}